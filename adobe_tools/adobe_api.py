...
class AdobeAPIObject(object):
  ...
  def __init__(
    ...
    'private_key_filename', 'userconfig', and 'cache_path' will default to the
    constants defined above if not provided.

    'cache' defaults to True to consume available cache data, and to store the
    data in local cache. False will not cache and ignores any local cache file.
    The cache path is defined in the constant above.

    'key' must be either 'email' or 'username', and determines what field
    to match the incoming data off of. By default, this is the 'email' field.
    This can be confusing because regardless of key choice, 'username' is used
    to indicate the unique user.
    """
    self.configs = {}
    self.productlist = []
    self.userlist = []
  ...
  def __prepare_jwt_token(self):
    ...
    payload = {
      ...
      "aud": "https://" + self.configs['ims_host'] + "/c/" +
             self.configs['api_key'],
      "https://" + self.configs['ims_host'] + "/s/" + "ent_user_sdk": True
    }
  ...
  def __generate_config(self, userconfig, private_key_filename):
    ...
    try:
      ...
      print >> sys.stderr, (
        "Cryptography module was unable to succeed on "
        "your machine.\n"
        "Please consult \n"
        "https://our.intern.facebook.com/intern/"
        "wiki/IT_Frontpage/CPE/Adobe_Creative_Cloud/#Troubleshooting \n"
        "for what to do next.")
      sys.exit(-1)
  ...
  def __read_cache(self):
    ...
    # If the cache file is older than 6 hours, disregard it
    cache_data = {}
    try:
      file_age = os.path.getmtime(self.cache_path)
      if time.time() - file_age < (60 * 60 * 6):
  ...
  def gather_product_list(self, force=False):
    ...
    Returns 'productlist', which is a list of dictionaries containing all the
    Configuration groups in use.
    If 'force' is true, the API call will be made regardless of cache.
    If a non-200 status code is returned by the API, an exception is raised.
    ...
    >>> api.productlist[1]
    {u'memberCount': 912,
    u'groupName': u'Default Document Cloud for enterprise - Pro Configuration'}
    ```
    """
  ...
  def gather_user_list(self, force=False):
    ...
    ```
    >>> api.userlist[0]
    {u'status': u'active', u'username': u'email@fb.com', u'domain': u'fb.com',
    u'firstname': u'Fake Firstname', u'lastname': u'Fake Lastname',
    u'groups': [u'Default Document Cloud for enterprise - Pro Configuration',
  ...
  def users_of_product(self, product_config_name):
    ...
    'userlist' is a list of dictionaries containing the user data of each user
    who is a member of that product configuration group.
    If a non-200 status code is returned by the API, an exception is raised.
  ...
  def update_user(self):
    ...
    self.gather_user_list(force=True)
    self.user = self.data()
    if self.cache:
      self.__write_cache()
  ...
  # ACTION FUNCTIONS
  # These functions are actions you can take on the user, which require posting
  # data to the API.
  def _submit_user_action_request(self, body_dict):
    ...
    if res.status_code != 200:
      ...
      if results.get('notCompleted') == 1:
        raise AdobeAPIIncompleteUserActionException(results.get('errors'))
