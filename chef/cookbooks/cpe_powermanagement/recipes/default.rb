...
ruby_block 'pm_prefs' do
  block do
    ...
    pm_prefs.keys.each do |type|
      ...
      energy_profile['PayloadContent'][0][
        "#{ident}.#{type}-ProfileNumber"] = -1
      energy_profile['PayloadContent'][0][
        "#{ident}.#{type}"] = pm_prefs[type]
      node.default['cpe_profiles']["#{prefix}.powermanagement"] =
        energy_profile
    end
  end
  action :run
